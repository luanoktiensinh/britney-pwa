...
export const useNavigation = (props = {}) => {
    ...
    const handleBack = useCallback(() => {
        ...
        if (parent) {
            setView(parent);
        } else if ((category || categoryId === 9999) && !isTopLevel) {
            setCategoryId(categoryId === 9999 ? Object.values(categories).find(cat => cat.name === 'Default Category').uid: category.parentId);
        } else {
            closeDrawer();
        }
