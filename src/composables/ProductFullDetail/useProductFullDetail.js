...
const deriveOptionSelectionsFromProduct = (product, valueKey = 'value_index') => {
    if (!isProductConfigurable(product)) {
        return INITIAL_OPTION_SELECTIONS;
    }
    const initialOptionSelections = new Map();
    let optionValue;
    for (const { attribute_id, attribute_code, values } of product.configurable_options) {
        optionValue = undefined;
        if(['color', 'fashion_color'].includes(attribute_code) && values?.length) {
            optionValue = values[0][valueKey];
        }
        initialOptionSelections.set(attribute_id, optionValue);
    }
...
export const useProductFullDetail = props => {
    ...
    const defaultOptionSelectionWithLabel = useMemo(
        () => deriveOptionSelectionsFromProduct(product, 'label'),
        [product]
    );
