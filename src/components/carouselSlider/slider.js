...
import defaultClasses from 'src/styles/pagebuilder/Slider/slider.module.css';
...
const Slider = props => {
    ...
    useEffect(() => {
        children?.length && children.forEach((child, index) => {
            child.props.data.isActive = index === 0
        });
    }, [children])
    return (
        <div
