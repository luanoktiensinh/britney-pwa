...
export const useCategory = props => {
    ...
    const sortProps = useSort({ sortFromSearch: false, isFirstLoad: true });
    const [currentSort] = sortProps;

    const initialSortValue = currentSort ? `${currentSort.sortAttribute}__${currentSort.sortDirection}` : "";
    const [queryParamsValues, queryParamsApi] = useQueryParams({initialValues: {pageSize: 24, sort: initialSortValue}});
    const { currentPageSize, currentSortParam } = queryParamsValues;
    const { setCurrentPageSize, setCurrentSortParam, setCurrentPage } = queryParamsApi;
    const [ isFirstLoad, setIsFirstLoad ] = useState(true);
    let categoryContentElem = null;
    ...
    useEffect(() => {
        ...
        if (!filterTypeMap.size || !currentPageSize) {
            return;
        }
        ...
        // Scroll to content wrapper element if currentPage is changed
        if(currentPage !== previousPage.current) {
            previousPage.current = currentPage;
            scrollToContentWrapperElem();
        }
    }, [
        currentPage,
        currentSort,
    ...
    useEffect(() => {
        ...
        if (
            ...
            (
                !previousSort.current.isFirstLoad && (
            ...
            if(!isFirstLoad) {
                scrollToContentWrapperElem();
            } else if (nextSearch.has("sort") && nextSearch.has("page_size")) {
                // check if search params includes all 3 params(page, sort and page_size) then change state of isFirstLoad to false
    ...
    const scrollToContentWrapperElem = () => {
        categoryContentElem?.scrollIntoView({
            behavior: 'smooth',
            block: 'start',
            inline: 'center'
        });
    ...
    const setContentWrapperRef = useCallback(elem => {
        categoryContentElem = elem;
    });
