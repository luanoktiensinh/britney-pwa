...
import DEFAULT_OPERATIONS from 'src/queries/CartPage/GiftCards/giftCardQueries.gql.js';
...
export const useGiftCards = props => {
    ...
    const applyGiftCard = useCallback(
        ...
        [formApiRef.current, applyCard, cartId]
    );

    const checkGiftCardBalance = useCallback(() => {
        setMostRecentAction(actions.CHECK_BALANCE);

        const giftCardCode = formApiRef.current.getValue('card');

        checkCardBalance({
            variables: { giftCardCode }
        });
    }, [formApiRef.current, checkCardBalance]);
