...
import { Link as HeadLink } from '@magento/venia-ui/lib/components/Head';
import {
    generateSrcset,
    generateUrl
...
const Banner = props => {
    ...
    const resourceImage = useResourceImage({
        generateSrcset,
        generateUrl,
        resource: image,
        type: 'image-product'
    ...
    let BannerFragment = (
        ...
        >
            {isActive && <HeadLink rel="preload" as="image" href={srcImage} imagesrcset={srcSetImage} imagesizes={sizesImage}></HeadLink>}
            <Image
                resource={image}
                alt="banner"
                classes={{
                    root: 'absolute top-0 left-0 w-full h-full',
                    container: '',
